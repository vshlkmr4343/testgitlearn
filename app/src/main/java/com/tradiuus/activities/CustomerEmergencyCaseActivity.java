...
import com.tradiuus.fragments.CustomerEmergencyReqConditionFragment;
import com.tradiuus.fragments.CustomerEmergencyReqConditionUploadFragment;
...
public class CustomerEmergencyCaseActivity extends BaseActivity implements CustomerImpl.OnCustomerGetTradeQuestionnaireListener, TechnicianImpl.OnTechnicianFundListener, ImageImpl.OnProblemImageUploadListener {
    ...
    protected void setActivityLayout() {
        ...
        setContentView(R.layout.activity_customer_emergency_case);
        this.context = CustomerEmergencyCaseActivity.this;
            ...
            case R.id.contractor_dashboard_logut:
                if (getSupportFragmentManager().getBackStackEntryCount() == 1) {
                    if (this.mService != null) {
                        replaceFragment(new CustomerEmergencyReqConditionUploadFragment());
                    } else {
    ...
    protected void initLoadCall() {
        try {
            ...
            customerImpl.getTradeQuestionnaire(selectedTrade.getTrade_id(), "1", "");
            //replaceFragment(new CustomerEmergencyReqFragment());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void getTradeQuestionnaire(Question questionnaire) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                replaceFragment(new CustomerEmergencyReqConditionFragment());
            }
        });
    }
    ...
    public void onServiceItemSelect(Service services) {
        ...
        this.mService.setService_type_id("1");
    }
    ...
    public void onImageUploadSuccess(ArrayList<String> images, String message, boolean directCall) {
        try {
            ...
            if (directCall) {
                ...
                startActivity(new Intent(context, CustomerFindEmergencyTechnicianActivity.class));
                overridePendingTransition(0, 0);
            }
        } catch (Exception e) {
            e.printStackTrace();
