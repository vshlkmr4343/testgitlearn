...
import com.tradiuus.fragments.TechnicianEstimateReqListDetailsFragment;
import com.tradiuus.fragments.TechnicianEstimateReqListFragment;
...
public class TechnicianEstimateRequestActivity extends BaseActivity implements TechnicianImpl.OnTechnicianJobCompletedListener {
    ...
    protected void setActivityLayout() {
        setContentView(R.layout.activity_contractor_estimate_req);
        EventBus.getDefault().register(this);
        StackActivities.registerActivity(this);
        this.context = TechnicianEstimateRequestActivity.this;
    ...
    protected void initLoadCall() {
        ...
        replaceFragment(new TechnicianEstimateReqListFragment());
    }
    ...
    public void completeJob(Job job, int index) {
        technicianImpl.completeJob(job.getCustomer_details().getCustomer_id(), mTechnician.getTechnician_id(), job.getJob_id(), "2", index);
    }
    ...
    public void onDataSetRefreshed() {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (getSupportFragmentManager().getFragments().size() > 0 && getSupportFragmentManager().getFragments().get(0) instanceof TechnicianEstimateReqListFragment) {
                    ((TechnicianEstimateReqListFragment) getSupportFragmentManager().getFragments().get(0)).refreshUI();
                }
            }
        });
    }
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        ((TradiuusApp) context.getApplicationContext()).estimateJobList.get(index).setJob_status(status);
                        if (fragment instanceof TechnicianEstimateReqListDetailsFragment) {
                            onBackPressed();
                            new Handler().postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        ((TechnicianEstimateReqListFragment) fragment).updateEstimateJob();
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                                }
